...
from Authentication.utils import ShortUUIDField

from Authentication.models import User, Profile, user_directory_path
...
class Post(models.Model):
    ...
    pid = ShortUUIDField(max_length=10)
    ...
    def save(self, *args, **kwargs): #method to slugify the title
        if not self.slug:
            random_digits = random_number()
            unique_id = str(random_digits)[:2] 
            if self.title:
                title_prefix = self.title[:4] 
            else:
                title_prefix = "untitled" 
            self.slug = f"{slugify(title_prefix)}-{unique_id}"
    ...
    def post_comments(self):
        comments = Comment.objects.filter(post=self, active=True).order_by('-id') #this would order by the latest
        return comments
...
class FriendRequest(models.Model):
    fid = ShortUUIDField(max_length=10)
...
class Comment(models.Model):
    ...
    date = models.DateTimeField(auto_now_add=True)
    cid = ShortUUIDField(max_length=10)
...
class Notification(models.Model):
    ...
    date = models.DateTimeField(auto_now_add=True)
    nid = ShortUUIDField(max_length=10)
...
class Group(models.Model):
    ...
    video = models.FileField(upload_to=user_directory_path, null=True, blank=True)
    visibility = models.CharField(max_length=10, default="everyone", choices=VISIBILITY)
    gid = ShortUUIDField(max_length=10)
    active = models.BooleanField(default=True)
    slug = models.SlugField(unique=True)
    views = models.PositiveIntegerField(default=0)
    date = models.DateTimeField(auto_now_add=True)
...
class ChatMessage(models.Model):
    ...
    is_read = models.BooleanField(default=False)
    date = models.DateTimeField(auto_now_add=True)
    mid = ShortUUIDField(max_length=10)
